...
import { changeSvgColor } from "../../app/font-icon/utils/changeIconColor";
...
interface IconProps {
  ...
  setCollection: React.Dispatch<
    React.SetStateAction<IconCollection | undefined>
  >;
  setErrorMessage: React.Dispatch<React.SetStateAction<string>>;
}
...
export const IconContainer = ({
  ...
  const handleResetColor = () => {
    setCollection((prev) => {
      if (prev) {
        ...
        newIcons.forEach((ico, index) => {
          const content = changeSvgColor(ico.content, "#" as string);
        ...
        if (file.type === "image/svg+xml") {
          ...
          reader.onload = (event: Event) => {
            if (collection) {
              ...
              setCollection((prev) =>
                prev ? { ...prev, icons: [...prev.icons, newIcon] } : undefined
              );
            }
          };
  ...
  const handleDownloadFonts = () => {
    if (collection) {
      ...
      window.location.replace(
        `https://extinct-houndstooth-fly.cyclic.cloud/api/v1/icons/download-fonts/${collection.name}`
      );
    }
  };
  ...
  const handleDownloadIcons = () => {
    if (collection) {
      ...
      window.location.replace(
        `https://extinct-houndstooth-fly.cyclic.cloud/api/v1/icons/download-icons/${collection.name}`
      );
    }
  };
            ...
            <div className="setup">
              ...
              <div className="buttons">
                <button
                  ...
                  data-tooltip
                  onClick={() =>
                    isEdit && unsaveHandler
                      ? unsaveHandler()
                      : saveHandler(collection)
                  }
                ...
                >
                  ...
                  <DescriptionTooltip
                    text={isEdit ? "Download fonts" : "Save to download fonts"}
                    tipPosition="bottom"
                  />
                </button>
...
const IconSet = ({ svg, handleAddIcon }: IconSetProps) => {
  ...
  return (
    <div className="collection-container">
      ...
      <div
        className={`icons-container ${!toggle ? "icons-container--close" : ""}`}
      >
          ...
          <div className="icon" key={`palette-${icn.name}`}>
            <img
              ...
              src={`data:image/svg+xml;base64,${btoa(
                unescape(encodeURIComponent(icn.content))
              )}`}
            />
        ...
        {itemsToShow < svg.icons.length && (
          <button className="showmore" onClick={handleShowMoreIcons}>
            Show more
          </button>
        )}
      </div>
