...
export function createColor(id: string): ColorType {
  ...
  const color: ColorType = {
    ...
    palettes: {
      analogous: makeAnalogousPalette({ color: hex, quantity: 4, format: 'hex' }) as string[],
      complementary: makeComplementaryPalette({ color: hex, quantity: 4, format: 'hex' }) as string[],
      monochromatic: makeMonochromaticPalette({ color: hex, quantity: 4, format: 'hex' }) as string[],
      ['split-complementary']: makeSplitComplementaryPalette({ color: hex, quantity: 4, format: 'hex' }) as string[],
      square: makeSquarePalette({ color: hex, quantity: 4, format: 'hex' }) as string[],
      tetradic: makeTetradicPalette({ color: hex, quantity: 4, format: 'hex' }) as string[],
      triadic: makeTriadicPalette({ color: hex, quantity: 4, format: 'hex' }) as string[],
    },
    colorBlind: {
